...
const infoController = async(req, res) => {
    ...
    try {
        ...
        const updateStatus = await ItemTransactions.updateTransactionStatus(transaction, DELIVERED)
        const updateData = await serialize(updateStatus)

        const userWallet = await UserWallet.getUserWalletById(user_id, false);
        const balance = await UserWallet.getBalance(userWallet);
        let ongkir = 5/100 * total
        await UserWallet.updateBalance(userWallet, balance+ongkir)
...
router.put('/confirm-delivered', validator, infoController)
