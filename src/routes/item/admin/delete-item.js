...
const chartController = async(req, res) => {
    ...
    try {
        const Item = new repositories.Item(connection);

        await Item.destroyUserWalletById(itemId)


    response = successResponse('Item delete', "Item Deleted Successfully");
  } catch (err) {
    logError(err);
...
router.delete('/delete-item', validator, chartController)
